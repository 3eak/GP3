...
<?php
	...
	$this->db->query("create temporary table temp as (select r.DriverName AS 'Driver',v.VehicleReg AS 'Vehicle',s.SupplierName AS 'Supplier',e.VenueStadiumName AS 'Venue',d.DeliveryDate AS 'Date'
	from delivery d
	JOIN supplier s ON (d.DeliverySupplier=s.SupplierID)
	JOIN driver r ON (d.DeliveryDriver=r.DriverID)
	JOIN vehicle v ON (d.DeliveryVehicleReg=v.VehicleID)
	JOIN venue e ON (d.DeliveryVenue=e.VenueID)
	where r.DriverID=d.DeliveryDriver AND s.SupplierID=d.DeliverySupplier AND d.DeliveryVehicleReg=v.VehicleID AND d.DeliveryVenue=e.VenueID AND d.DeliverySupplier='3' AND d.DeliveryDate=(DATE '2018/10/20'))");
